...
func createTestUser(t *testing.T, pool *pgxpool.Pool, googleSub string) int {
	t.Helper()
	...
	err := pool.QueryRow(context.Background(), `
		...
		VALUES ($1, $2, $3, '')
		RETURNING id
	`, googleSub, googleSub+"@example.com", "Test User").Scan(&userID)
	require.NoError(t, err)
	return userID
}
...
func TestHabitService(t *testing.T) {
	...
	ctx := context.Background()
	userID := createTestUser(t, pool, "google-sub-1")
	today := time.Now()
	yesterday := today.AddDate(0, 0, -1)

	t.Run("Create", func(t *testing.T) {
		h, err := svc.Create(ctx, userID, "Read", "Read for 30 minutes", 1, "day")
		require.NoError(t, err)
	...
	t.Run("List", func(t *testing.T) {
		habits, err := svc.List(ctx, userID, today)
		require.NoError(t, err)
	...
	t.Run("Toggle complete and uncomplete", func(t *testing.T) {
		h, err := svc.Create(ctx, userID, "Exercise", "", 1, "day")
		require.NoError(t, err)

		hasEntry, err := svc.Toggle(ctx, userID, h.ID, today)
		require.NoError(t, err)
	...
	t.Run("Delete hides habit from today", func(t *testing.T) {
		h, err := svc.Create(ctx, userID, "Temporary", "", 1, "day")
		require.NoError(t, err)

		err = svc.Delete(ctx, userID, h.ID, today)
		require.NoError(t, err)
	...
	t.Run("Delete preserves habit in past dates", func(t *testing.T) {
		h, err := svc.Create(ctx, userID, "Was active yesterday", "", 1, "day")
		require.NoError(t, err)
		setHabitCreatedAt(t, pool, h.ID, yesterday.AddDate(0, 0, -1))

		_, err = svc.Toggle(ctx, userID, h.ID, yesterday)
		require.NoError(t, err)
		...
		todayHabits, err := svc.List(ctx, userID, today)
		require.NoError(t, err)
		...
		pastHabits, err := svc.List(ctx, userID, yesterday)
		require.NoError(t, err)
		var found bool
	...
	t.Run("Weekly habit period_count and completion", func(t *testing.T) {
		h, err := svc.Create(ctx, userID, "Gym", "", 2, "week")
		require.NoError(t, err)
		setHabitCreatedAt(t, pool, h.ID, yesterday.AddDate(0, 0, -1))

		// one entry — not yet complete for the week
		_, err = svc.Toggle(ctx, userID, h.ID, today)
		require.NoError(t, err)
	...
	t.Run("Streak increments on consecutive days", func(t *testing.T) {
		h, err := svc.Create(ctx, userID, "Streak habit", "", 1, "day")
		require.NoError(t, err)
		setHabitCreatedAt(t, pool, h.ID, yesterday.AddDate(0, 0, -1))
		...
		h2, err := svc.Create(ctx, userID, "Day-one streak", "", 1, "day")
		require.NoError(t, err)
		_, err = svc.Toggle(ctx, userID, h2.ID, today)
		require.NoError(t, err)
	...
	t.Run("Weekly streak: in-progress week does not break past streak", func(t *testing.T) {
		h, err := svc.Create(ctx, userID, "Gym weekly", "", 3, "week")
		require.NoError(t, err)
		...
		for _, d := range []int{3, 4, 5} {
			_, err = svc.Toggle(ctx, userID, h.ID, time.Date(2025, 3, d, 0, 0, 0, 0, time.UTC))
			require.NoError(t, err)
		}
		...
		_, err = svc.Toggle(ctx, userID, h.ID, time.Date(2025, 3, 17, 0, 0, 0, 0, time.UTC))
		require.NoError(t, err)

		habits, err := svc.List(ctx, userID, ref)
		require.NoError(t, err)
	...
	t.Run("Weekly streak includes current week when completed", func(t *testing.T) {
		h, err := svc.Create(ctx, userID, "Gym current week", "", 2, "week")
		require.NoError(t, err)
		setHabitCreatedAt(t, pool, h.ID, time.Date(2025, 3, 1, 0, 0, 0, 0, time.UTC))
		...
		_, err = svc.Toggle(ctx, userID, h.ID, time.Date(2025, 3, 10, 0, 0, 0, 0, time.UTC))
		require.NoError(t, err)
		_, err = svc.Toggle(ctx, userID, h.ID, time.Date(2025, 3, 11, 0, 0, 0, 0, time.UTC))
		require.NoError(t, err)
		// Current week: 2 entries (completed)
		_, err = svc.Toggle(ctx, userID, h.ID, time.Date(2025, 3, 17, 0, 0, 0, 0, time.UTC))
		require.NoError(t, err)
		_, err = svc.Toggle(ctx, userID, h.ID, time.Date(2025, 3, 18, 0, 0, 0, 0, time.UTC))
		require.NoError(t, err)
	...
	t.Run("Monthly streak: in-progress month does not break past streak", func(t *testing.T) {
		h, err := svc.Create(ctx, userID, "Monthly review", "", 2, "month")
		require.NoError(t, err)
		...
		_, err = svc.Toggle(ctx, userID, h.ID, time.Date(2025, 1, 10, 0, 0, 0, 0, time.UTC))
		require.NoError(t, err)
		_, err = svc.Toggle(ctx, userID, h.ID, time.Date(2025, 1, 20, 0, 0, 0, 0, time.UTC))
		require.NoError(t, err)
		// February: 2 entries (completed)
		_, err = svc.Toggle(ctx, userID, h.ID, time.Date(2025, 2, 5, 0, 0, 0, 0, time.UTC))
		require.NoError(t, err)
		_, err = svc.Toggle(ctx, userID, h.ID, time.Date(2025, 2, 15, 0, 0, 0, 0, time.UTC))
		require.NoError(t, err)
		// March: 1 entry (in progress)
		_, err = svc.Toggle(ctx, userID, h.ID, time.Date(2025, 3, 1, 0, 0, 0, 0, time.UTC))
		require.NoError(t, err)
	...
	t.Run("History: past date shows past entry", func(t *testing.T) {
		h, err := svc.Create(ctx, userID, "History habit", "", 1, "day")
		require.NoError(t, err)
		setHabitCreatedAt(t, pool, h.ID, yesterday.AddDate(0, 0, -1))
